...
model3.lnorm <- bugs( data, inits1, model.file = "modellnorm.txt",
...
print(model3.lnorm,3) #model with 3 digits
#
names(model3.lnorm)
...
plot.trace( model3.lnorm,3,2)

# ergodic mean plots - in a window with 3 rows and 2 columns
plot.trace( model3.lnorm,3,2, ergodic=T)
...
par(mfrow=c(2,1))
# Trace plot of  "mu"
plot(model3.lnorm$sims.matrix[,"mu"],type='l',main="Trace plot of mu"
		,xlab="iterations",ylab="mu")

# Trace plot of  "tau"
plot(model3.lnorm$sims.matrix[,"tau"],type='l',main="Trace plot of tau"
...
par(mfrow=c(2,1))
#acf plots
acf(model3.lnorm$sims.matrix[,"mu"],main="ACF plot of mu",lag=100)
acf(model3.lnorm$sims.matrix[,"tau"],main="ACF plot of tau",lag=100)
...
plot(density(model3.lnorm$sims.matrix[,"mu"]),main="mu")

# Posterior density of "tau" 
plot(density(model3.lnorm$sims.matrix[,"tau"]),main="tau")

# Posterior density of "s2" 
plot(density(model3.lnorm$sims.matrix[,"s2"]),main="s2")
...
modellnorm.sim<-read.coda("output.odcCODAchain1", "output.odcCODAindex", quiet=FALSE) 	
